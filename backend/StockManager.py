...
API_KEY = '7HGJ26FC4S808C9P'
API_KEY_1 = '7KPROM2JV9JUG3AH'
BASE_URL = 'https://www.alphavantage.co/query'


def get_price(strategies):

    latest_price = {}
    for strategy in strategies:
        for stock in stocks[strategy]:
            url = BASE_URL + '?function=GLOBAL_QUOTE&symbol={}&apikey='+API_KEY
            response = requests.get(url.format(stock))
            if response.status_code != 200:
                Exception("API Error")
            response_json = response.json()
            latest_price[stock] = {"price": response_json["Global Quote"]['05. price'], "strategy": get_strategy_by_stock(stock)}
...
def allocate_stocks(amount, strategies):
    ...
    latest_price = get_price(strategies)
